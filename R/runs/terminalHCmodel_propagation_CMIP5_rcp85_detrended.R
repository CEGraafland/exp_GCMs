...
loadIterations <- function(pattern,permused, hctype = NULL, ncep = FALSE, it = NULL){
  ...
  if(ncep == FALSE){fullpattern <- paste0("data/hciterations_detrended/",pattern)} else {fullpattern <- pattern}
...
data.future_gcms_out <- lapply(cubicsets.future,function(x) TimeCoordsAnom_from_Grid_rms(detrendGrid(x), rms = TRUE))
...
hc_gcms.future <- c(hc_gcms5.rcp85)
names(hc_gcms.future) <- c(namescubics5.rcp85)
