...
grid <- detrendGrid(grid_gcms_anom_scaled$interim_10d_akima_cubic)
grid <- grid_gcms_anom_scaled$interim_10d_akima_cubic
grid <- detrendGrid(grid_gcms_anom_scaled$CMIP5_MIROC.ESM_r1i1p1_rcp85)
grid <- grid_gcms_anom_scaled$CMIP5_MIROC.ESM_r1i1p1_rcp85
grid <- grid_gcms_anom_scaled$CMIP5_MIROC.ESM_r1i1p1


temporalPlot(grid)
temporalPlot(aggregateGrid(grid,aggr.y = list(FUN ="mean")))
temporalPlot(subsetGrid(grid, season =8))


glob.timeseries <- aggregateGrid(grid, aggr.spatial = list(FUN = "mean"))
plot(glob.timeseries$Data)
quantity2clim(TimeCoordsAnom_from_Grid(grid),ref.grid = grid, what = "timeseries")
...
dev.off()
#####################################################################################################
# Plot propagation fields with respect to clustering without 2x times earth
#####################################################################################################
load("results/hellinger_coefficient/hellinger_coefficients_CMIP5_CMIP6_hist_vs_fut.rda")
selecteddists <- hellinger_coefficients
mat<- -log(selecteddists)
...
earth.reans.ids <- sapply(c("CMIP5_EC.EARTH_r1i1p1","CMIP5_EC.EARTH_r2i1p1","interim_10d_akima_cubic","ncep_10d","JRA55_10d_akima_cubic"),function(x)which(rownames(submat.hist5) == x ))
submat.hist5 <- submat.hist5[-earth.reans.ids,-earth.reans.ids]
longdata <- reshape2::melt(submat.hist5)

mat[which(mat == Inf, arr.ind = TRUE)]<- 100
matclust <-hclust(dist(submat.hist5, method = "minkowski", p = 2),method = "complete")
matclust <-hclust(dist(submat.hist5, method = "euclidean"),method = "complete")

matclust$labels[matclust$order]

binds <- prop_dif_pos_and_neg[c(matclust$labels[matclust$order],"interim_10d_akima_cubic","ncep_10d","JRA55_10d_akima_cubic")]
compare.all <- bindGrid(binds, dimension = "member")
#compare.all.int <- bindGrid(prop_dif_pos_and_neg[matclust$labels[matclust$order]],compare.all,dimension = "member",skip.temporal.check = TRUE)


plotname <-  paste0("figs/evidence_propagation/CMIP5_cluster_propagation/CMIP5cluster_reansapart_",whichiterations,"_plus",whichnode,"pos.pdf")
pdf(plotname,width= 10, height= 10)


spatialPlot(compare.all,names.attr = c(matclust$labels[matclust$order],"interim_10d_akima_cubic","ncep_10d","JRA55_10d_akima_cubic"),
            as.table = TRUE,
            # layout = c(1,33),
            cex = 0.01,
            backdrop.theme = "coastline", lonCenter = 180, main = list(paste0(attr(prop_dif_pos_and_neg[[1]]$Data,"climatology:fun"))), at = seq(-0.85,0.85,0.01),region = TRUE, 
...
plotname <-  paste0("figs/evidence_propagation/CMIP5_cluster_propagation/CMIP5cluster_reansapart_dendogram",whichiterations,"_plus",whichnode,"pos.pdf")
pdf(plotname,width= 20, height= 7)
...
##############################################################################
namesselected <- c(rownames(submat.hist5[matclust$order,matclust$order]),"interim_10d_akima_cubic","ncep_10d","JRA55_10d_akima_cubic")
namesselected <- c(rownames(submat.hist5[matclust$order,matclust$order]),"interim_10d_akima_cubic","JRA55_10d_akima_cubic")
...
orderedmat.cmips <- mat[namesselected,namesselected]
