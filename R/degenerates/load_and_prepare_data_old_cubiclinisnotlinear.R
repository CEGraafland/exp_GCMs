...
datasets <- UDG.datasets("CMIP5.*historical")$name[1]
datasets.85 <- UDG.datasets("CMIP5.*rcp85")$name[1]
...
# Por lo tanto he completado la serie histórica de cmip5 con los años 2006:2010 del rcp85
...
tas_gcm05s_10d[[1]]
spatialPlot(climatology(tas_gcm05s_10d_ak[[1]], clim.fun = list(FUN = "mean")), backdrop.theme = "coastline",lonCenter = 180)
spatialPlot(climatology(tas_gcm05s_10d_fi[[1]], clim.fun = list(FUN = "mean")), backdrop.theme = "coastline",lonCenter = 180)

spatialPlot(climatology(tas_interim_10dnew, clim.fun = list(FUN = "mean")), backdrop.theme = "coastline",lonCenter = 180)



str(historical[[1]])
which(is.na(historical[[1]]$Data[1,1,,,]) == FALSE)
historical[[1]]$xyCoords


tas_gcm05s_10d_ak <- lapply(historical, function(x){interpGrid(x,
                                                               new.coordinates = getGrid(tas_interim_10dnew),
                                                               method = 'bilinear',
                                                               bilin.method = 'akima'
)
})
tas_gcm05s_10d_fi <- lapply(historical, function(x){interpGrid(x,
                                                               new.coordinates = getGrid(tas_interim_10dnew),
                                                               method = 'bilinear',
                                                               bilin.method = 'fields'
)
